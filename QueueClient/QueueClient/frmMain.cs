...
namespace QueueClient
{
    public partial class frmMain : Form
    {
        ...
        public frmMain()
        {
            ...
            pnWork.Size = new Size(1920, 1080);
            pnWork.Location = new Point(0, 0);
            ...
            pnPhone.Size = new Size(1920, 1080);
            pnConsult.Size = new Size(1920, 1080);
            pnPhone.Location = new Point(0, 0);
            pbReturnMain.Location = new Point(200, 75);
            pbLastPage.Location = new Point(1550, 75);
            pnBackGround.Size = new Size(1920, 1080);
        ...
        private void pt1_Click(object sender, EventArgs e)
        {
            ...
            if (No == "b")
            {
                if (txtPhoneNo.Text.Length > 0 && txtPhoneNo.Text.Length <= 11)
                {
                    txtPhoneNo.Text = txtPhoneNo.Text.Substring(0, txtPhoneNo.Text.Length - 1);
            ...
            else if (No == "f")
            {
                ...
                {
                    frmMsg frm = new frmMsg();//提示
                    frm.msgInfo = "手机号位数不正确！";
                    frm.ShowDialog();
                    return;
                }
                else
                {
                    mobilePhone = txtPhoneNo.Text;
                    var register = RegisterUser.Replace("@idCard", idCard);
                    register = register.Replace("@mobilePhone", mobilePhone);
                    if (person != null && person.name != null && person.name != "")
                        register = register.Replace("@name", person.name);
                    var jsonString = http.HttpGet(register, "");
                    var User = script.DeserializeObject(jsonString) as Dictionary<string, object>;
                    if (User != null)
                    {
                        var status = User["status"].ToString();
                        var dataArr = User["data"] as Dictionary<string, object>;
                        var desc = User["desc"].ToString();
                        if (status == "200")
                        {
                            userCode = dataArr["userCode"].ToString();
                            //成功
                            TRegisterModel reg = new TRegisterModel();
                            reg.custCardId = idCard;
                            reg.userCode = mobilePhone;
                            reg.mobilePhone = mobilePhone;
                            reg.registerDateTime = DateTime.Now;
                            reg.registerAddress = "";//暂时为空
                            rBll.Insert(reg);
                            SelectUnit();
                        }
                        else
            ...
            else
            {
                if (txtPhoneNo.Text.Length < 11)
                {
                    txtPhoneNo.Text = txtPhoneNo.Text + No;
                    txtPhoneNo.SelectionStart = txtPhoneNo.Text.Length;
                }
            }
        ...
        {
            this.pnPhone.BringToFront();
            pbReturnMain.BringToFront();
            pbLastPage.BringToFront();
            txtPhoneNo.Text = "";
            Start(false);
            pageLocation = PageLocation.WorkInputPhone;
        }
